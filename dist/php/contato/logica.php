<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if(isset($_POST['name']) && isset($_POST['email']) && isset($_POST['about']) && isset($_POST['message'])) {
			$name = $_POST['name'];
			$email = $_POST['email'];
			$about = $_POST['about'];
			$message = $_POST['message'];
			$array = array($name, $email, $about, $message);
			...
			if($contact) {
																	...
																	<tr>
																		<td valign="top">
																			<table bgcolor="#ffffff" border="0" cellpadding="0" cellspacing="0"
																				...
																				<tbody>
																					<tr>
																						<td height="23" style="font-family: Arial, Helvetica, sans-serif; font-size: 30px; letter-spacing: 0; line-height: 32px; ;text-align: center; color: #404040;">Olá, '.$name.'!<br/><br/>Estamos quase lá!</td>
																					</tr>
																				</tbody>
																			</table>
																		</td>
				...
				if(!$mail->Send()){
					...
					echo(json_encode(['success' => false, 'title' => 'Erro!', 'message' => 'Ocorreu algum erro inesperado, por favor, tente novamente mais tarde. Log do Erro: '.$message, 'btnText' => 'Ok']));
				} else {
					echo(json_encode(['success' => true, 'title' => 'Sucesso!', 'message' => 'Obrigado por enviar sua mensagem! Em breve você receberá um e-mail de confirmação.', 'btnText' => 'Ir para o Feed']));
				}
